...
============================== */
let allCommandes = [];
let allProduits = []; // متغير جديد لحفظ المنتجات
...
window.openEditProductModal = openEditProductModal; // جديد
window.closeEditProductModal = closeEditProductModal; // جديد
window.saveProduct = saveProduct; // جديد
window.updateProduct = updateProduct; // جديد
window.deleteProduct = deleteProduct; // جديد
window.clearFilters = clearFilters;
window.filterCommandes = filterCommandes;
window.loadProduits = loadProduits; // جديد

// ========== CHARGEMENT DES COMMANDES ==========
function loadCommandes() {
  const tbody = document.getElementById('ordersTableBody');
  if (!tbody) { console.error("❌ ordersTableBody non trouvé"); return; }
  tbody.innerHTML = '<tr><td colspan="8" style="text-align:center; padding:40px;"><i class="fas fa-spinner fa-spin fa-2x"></i><br><br>Chargement...</td></tr>';
  ...
  db.collection("commandes").orderBy("date", "desc").get().then((snapshot) => {
    ...
    tbody.innerHTML = `<tr><td colspan="8" style="text-align:center; padding:40px; color:#e74c3c;">❌ ${error.message}</td></tr>`;
  });
}

// ========== CHARGEMENT DES PRODUITS (جديد) ==========
...
function displayCommandes(commandes) {
  ...
  if (commandes.length === 0) {
    tbody.innerHTML = `<tr><td colspan="8" style="text-align:center; padding:40px; color:#999;">📭 Aucune commande</td></tr>`;
    return;
  }
  tbody.innerHTML = commandes.map(cmd => `
    <tr>
      ...
      <td><span class="order-type">${cmd.orderType === 'domicile' ? '🏠 Domicile' : '🏪 Stop Desk'}</span></td>
      <td>${cmd.wilaya || 'N/A'}</td>
      <td>${cmd.phone1 || 'N/A'}</td>
      <td class="total-price">${(cmd.grandTotal || 0).toFixed(2)} DA</td>
      <td><span class="status-badge-table ${getStatusClass(cmd.status || 'pending')}">${getStatusLabel(cmd.status || 'pending')}</span></td>
      <td>
        <button class="action-btn" onclick="showDetail('${cmd.orderNumber}')"><i class="fas fa-eye"></i> Détails</button>
        <button class="delete-btn" onclick="deleteCommande('${cmd.orderNumber}')"><i class="fas fa-trash"></i></button>
      </td>
    </tr>
  `).join('');
}
...
function showDetail(orderNumber) {
  ...
  if(badge) { badge.textContent = getStatusLabel(status); badge.className = 'status-badge-table ' + getStatusClass(status); }
  ...
  if(items) {
    ...
    } else { items.innerHTML = '<p style="text-align:center;color:#999;">📭 Aucun produit</p>'; }
  }
...
// ========== 🛒 GESTION DES PRODUITS (URL GitHub) ==========
...
function openEditProductModal(productId) {
  ...
  if(preview) {
    if(produit.image) {
      preview.innerHTML = `<img src="${produit.image}" alt="${produit.name}" style="max-width:100%;max-height:200px;border-radius:8px;">`;
...
async function saveProduct(e) {
  ...
  if(!name||!category||!price||!imageUrl||isNaN(price)||price<=0) {
    alert("⚠️ Remplissez tous les champs obligatoires (Image = URL GitHub)"); return;
  }
  
  // تحقق بسيط من رابط GitHub
  if(!imageUrl.includes('github') && !imageUrl.includes('raw.githubusercontent')) {
    if(!confirm("⚠️ L'URL ne semble pas être un lien GitHub. Continuer quand même?")) return;
  }
  
  try {
    showNotification('📤 Ajout en cours...');
    await db.collection("produits").add({
      name, image: imageUrl, category, description: description||'', price,
    ...
    document.getElementById('addProductForm')?.reset();
    document.getElementById('imagePreview').innerHTML = '';
    loadProduits(); // تحديث قائمة المنتجات
  } catch(err) {
...
async function updateProduct(e) {
  ...
  if(!name||!category||!price||!imageUrl||isNaN(price)||price<=0) {
    alert("⚠️ Remplissez tous les champs"); return;
  }
  
  try {
    showNotification('🔄 Mise à jour...');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if(urlInput) {
    urlInput.addEventListener('input', function() {
      ...
      if(pv && this.value.trim()) {
        pv.innerHTML = `<img src="${this.value.trim()}" alt="Preview" style="max-width:100%;max-height:200px;border-radius:8px;">`;
...
function showNotification(msg) {
  ...
  n.textContent = msg;
  n.style.cssText = `position:fixed;top:20px;right:20px;background:linear-gradient(135deg,#10b981,#059669);color:white;padding:15px 25px;border-radius:10px;z-index:9999;font-weight:600;animation:slideIn 0.3s ease;`;
